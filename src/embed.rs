...
//!   Non-loopback endpoints require `privacy.allow_remote_endpoints`.
    ...
    match config.embeddings.provider.as_str() {
        ...
        other => bail!(
            "unknown embeddings.provider \"{other}\" \
             (expected \"embedded\", \"openai-compatible\", or \"debug-hash\")"
        ),
    }
}
...
mod tests {
    ...
    fn config_with(provider: &str, base_url: Option<&str>, allow_remote: bool) -> Config {
        ...
        c.privacy.allow_remote_endpoints = allow_remote;
        c
    }
    ...
    fn debug_hash_scores_shared_words_higher() {
        ...
        let docs = p
            .embed_docs(&[
                "closing statement house sale".into(),
                "quarterly budget review".into(),
            ])
            .unwrap();
        let q = p.embed_query("house sale").unwrap();
    ...
    fn remote_endpoint_requires_the_privacy_flag() {
        let dir = tempfile::tempdir().unwrap();
        let cfg = config_with("openai-compatible", Some("https://api.example.com/v1"), false);
        ...
        let cfg = config_with("openai-compatible", Some("https://api.example.com/v1"), true);
