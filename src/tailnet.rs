...
mod tests {
    ...
    fn serve_proxy_is_found_for_the_matching_backend() {
        assert_eq!(
            parse_serve_proxy(SERVE, "127.0.0.1:8788").as_deref(),
    ...
    fn serve_proxy_ignores_other_backends_and_bad_json() {
        ...
        assert_eq!(parse_serve_proxy("{}", "127.0.0.1:8788"), None);
        assert_eq!(parse_serve_proxy("nope", "127.0.0.1:8788"), None);
    }

    #[test]
    fn serve_proxy_keeps_non_root_mounts() {
        let nested = SERVE.replace(r#""/":"#, r#""/icloud":"#);
        assert_eq!(
            parse_serve_proxy(&nested, "127.0.0.1:8788").as_deref(),
            Some("https://mac.tail1234.ts.net:8443/icloud")
        );
    }
}
